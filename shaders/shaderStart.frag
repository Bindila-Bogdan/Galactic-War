...
uniform bool generateFog;
uniform bool computeAttenuation;
uniform bool turnOnFlashLight;
...
{			
	...
	if(turnOnFlashLight){
		float theta = dot(viewDirN , normalize(-cameraTarget)); 
    		float epsilon = (spotLight.x - spotLight.y);
    		float intensity = clamp((theta - spotLight.x) / epsilon, 0.0, 1.0);
		intensity *= 4.0f;    	
		diffuse  *= intensity;
    		specular *= intensity;
	}

	if(computeAttenuation){
...
void main() 
{
	...
	fColor = fogColor * (1 - fogFactor) + vec4(color, 1.0f);
    //fColor = vec4(o, 1.0f);
}
